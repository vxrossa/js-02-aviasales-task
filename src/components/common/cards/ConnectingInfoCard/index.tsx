...
const ConnectingInfoCard: FC<ISegment> = (segment) => {
  ...
  return (
    <StyledSegmentGrid>
      ...
      <StyledSegmentColumn>
        <StyledUpperText>{origin} - {destination}</StyledUpperText>
        <StyledLowerText>{startHour}:{startMinute} - {endHour}:{endMinute}</StyledLowerText>
      </StyledSegmentColumn>
      ...
      <StyledSegmentColumn>
        <StyledUpperText>{stops.length} {stopsText()}</StyledUpperText>
